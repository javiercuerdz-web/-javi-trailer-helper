"use strict";
const VERSION="9.4.0", KEY="javiTrailerHelperDataV9", DAILY_GOAL=540;
...
document.addEventListener("click",e=>{const b=e.target.closest("button");if(!b)return;if(b.dataset.edit)openDialog(b.dataset.edit);if(b.dataset.complete)openComplete(b.dataset.complete);if(b.dataset.copy!==undefined)copy(b.dataset.copy,b.dataset.label);if(b.dataset.removeSensor)removeSensor(b.dataset.trailerId,b.dataset.removeSensor);});
